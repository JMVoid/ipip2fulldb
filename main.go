...
func init()  {
	flag.StringVar(&ipipFile, "s", "china_ip_list.txt", "specify source ip list file")
	flag.StringVar(&dbSrcFile, "m", "Country-src.mmdb", "specify source mmdb file")
...
func main()  {
	...
	// start source mmdb file parse
	db, err := maxminddb.Open(dbSrcFile)
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
	...
	for networks.Next() {
		...
		if record.Country.IsoCode != "CN" {
			err = writer.Insert(subnet, parseCountry(record))
			if err != nil {
				log.Fatalf("fail to insert to writer %v\n in mmdb source", err)
			}
		}
